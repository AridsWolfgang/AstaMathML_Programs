...
static void mix_columns(uint8_t state[16])
{
    ...
    {
        int c = i * 4;
        uint8_t a0 = state[c+0], a1 = state[c+1], a2 = state[c+2], a3 = state[c+3];
        state[c+0] = gf_mul(2,a0) ^ gf_mul(3,a1) ^ a2 ^ a3;
...
static void inv_mix_columns(uint8_t state[16])
{
    for (int i = 0; i < 4; i++)
    {
        int c = i * 4;
        uint8_t a0 = state[c+0], a1 = state[c+1], a2 = state[c+2], a3 = state[c+3];
        state[c+0] = gf_mul(14,a0) ^ gf_mul(11,a1) ^ gf_mul(13,a2) ^ gf_mul(9,a3);
...
static void key_expansion(const uint8_t key[16], uint8_t rk[11][16])
{
    for (int i = 0; i < 16; i++) rk[0][i] = key[i];
    for (int r = 1; r <= 10; r++)
    {
        uint8_t temp[4];
        temp[0] = sbox[rk[r-1][13]] ^ rcon[r];
        temp[1] = sbox[rk[r-1][14]];
        temp[2] = sbox[rk[r-1][15]];
        temp[3] = sbox[rk[r-1][12]];
        for (int i = 0; i < 4; i++)
